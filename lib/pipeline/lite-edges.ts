...
export type LiteGraphFile = {
  id: string;
  path: string;
  summary: string | null;
};

function byRole(files: LiteGraphFile[]): Map<ArchRole, LiteGraphFile[]> {
  const m = new Map<ArchRole, LiteGraphFile[]>();
...
function getImports(file: LiteGraphFile): string[] {
...
export function buildLiteEdges(fileNodes: LiteGraphFile[]): LiteGraphEdge[] {
  ...
  const byDir = new Map<string, LiteGraphFile[]>();
  for (const ui of uis) {
